...
HRESULT player::init()
{
	...
	speedX = 10.0f;							// X Speed
	speedY = 4.5f;							// Y Speed
	posX = WINSIZEX / 2;					// Player coordinates
	posY = WINSIZEY / 2+280;				// Player coordinates
	jump_power = 0.8f;						// Jump Olak Speed
	gravity_accumulate = 0;					// Gravitational cumulative value
	gravity = 0.4f;							// Gravity
	_count = 0;								// Movement Count
	_index = 0;								// Movement frame
	_atk_count = 0;							// Attack Motion Cow
	_atk_index = 0;							// Attack Motion Frame
	_state = IDLE;							// Player Status
	_p_skill = Basic_Attack;				// Skill Type
	_rc = RectMake(posX,posY,150,200);		// Player Rack
	_skill_count = 0;						
	gravity1 = 0;
	timeHp = 0;
	isHp = false;
	iscoillision = false;					// Boss Bob Clash
	isJump = true;							// Check the jump status
	Right_and_left = false;					// Left and right movements
	_max_jump = false;						// Jump to reach the maximum position
	isJavelin = false;						// Spear Throw
	...
	_rc_sphear = RectMake(posX, posY,243, 19); // Javelin Rack
...
/* vector<RECT> _atkRc;			// Attack Raid
	vector<RECT> sh_Rc;			// Spear conflicts */
