...
{
	gravity1 = 0;
	speedX = 35.0f;							// X Speed
	speedY = 35.5f;							// Y Speed
	posX = WINSIZEX-100;					// Player coordinates
	posY = WINSIZEY / 2+83;					// Player coordinates
	...
	angle = 90;								// Boss jump angle
	_rc = RectMake(posX, posY, 624, 400);	// Boss Rack
	rendom_atk = 0;
	atk_count[2] = { 0 };
	atk_index[2] = { 0 };
	pattern_count = 0;
	_state  = B_IDLE;						// Stay
	_skill = B_Basic_Attack;				// Basic Attacks
	...
	isbossHpCount = false;
	isDown2 = false;
	isJump = false;							// Check the jump status
	Right_and_left = true;					// Left and right movements
	_max_jump = false;						// Jump to reach the maximum position
	atk_two = false;						// Basic Attacks
	isBasic_atk = false;					// During the basic attack
	jump_start = false;						// Preparing to Jump
	isHp = false;
	isDown3 = false;
	isDown5 = false;
	return S_OK;
